...
static void compound_operator(LexState *ls, LHSVarList *lh, BinOpr op) {
    ExpDesc rh;
    ExpDesc tmp = lh->v;
    ...
    /* Sum, reserve an register so the result will not overwrite lh->v (tmp) */
    bcemit_binop_left(ls->fs, op, &rh);
    bcreg_reserve(ls->fs, 1);
    bcemit_binop(ls->fs, op, &tmp, &rh);

    /* Store the right-hand+left-hand sum into the left-hand */
    bcemit_store(ls->fs, &lh->v, &tmp);
}
